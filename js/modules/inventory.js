...
import { upsert, softDelete, listActive, byId, newId, formatMoney, formatEUR, toEUR, totalRemaining } from '../core/data.js';
...
// Converts each batch's value to EUR before summing (via each batch's own
// currency/dateBought) — a batch in a non-EUR currency previously got summed
// as a raw number alongside EUR batches, silently mixing currencies whenever
// an item had batches bought in more than one currency.
function totalValue(item) {
  return (item.batches || []).reduce((s, b) => s + toEUR((b.remaining ?? b.qty ?? 0) * (b.unitPrice || 0), b.currency || 'EUR', b.dateBought), 0);
}
...
const INV_LIST_COLS = [
  ...
  { label: 'Value',         right: true, tip: 'Total value of remaining stock in EUR: each batch\'s remaining quantity × its unit price, converted to EUR, summed.' },
...
function build() {
  ...
  const render = () => {
    ...
    for (const item of rows) {
      ...
      const valueTd = el('td', { class: 'right num', style: 'display:flex;align-items:center;justify-content:flex-end;gap:4px' }, formatEUR(value));
      valueTd.appendChild(mkExplainButton({
        title: 'Value',
        formula: 'Σ (batch remaining × batch unit price, converted to EUR), summed across all batches.',
        inputs: [
          { label: 'Batches counted', value: String(batches.length) },
          { label: 'Total Value', value: formatEUR(value) }
        ],
        source: 'js/modules/inventory.js totalValue()',
        note: 'Each batch is converted to EUR via its own currency and purchase date before summing, so batches bought in different currencies add up correctly.'
      }));
